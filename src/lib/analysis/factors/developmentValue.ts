...
function compute(ctx: FactorContext): FactorComputation {
  ...
  if (age !== null) {
    if (age <= 20) ageAdj = 24;
    else if (age <= 23) ageAdj = 16;
    else if (age <= 26) ageAdj = 5;
  ...
  const score = clamp(56 + ageAdj + headroomAdj + trajAdj);
  // Real PL data (or FC) makes this a usable signal; only truly data-less players are low-confidence.
  const confidence: FactorComputation["confidence"] =
    (player.advanced || fc) && player.matchConfidence !== "none" ? (age !== null ? "high" : "medium") : "low";
