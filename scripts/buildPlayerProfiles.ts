...
import { STYLE_KEYS, styleValueAtTier, TIER_ORDER } from "../src/lib/analysis/styleDimensions";
import { buildAdvancedIndex } from "../src/lib/pl/advancedStats";
import { buildClubDna } from "../src/lib/pl/clubDna";
import { plClubKey } from "../src/lib/pl/plClubs";
import type { LeagueBaselines, DimTier } from "../src/lib/types";
...
function parseNetTransfer(raw: string | undefined): number | null {
  if (!raw) return null;
  const m = String(raw).replace(/\s/g, "").match(/([+-]?)\D*([\d.]+)\s*([mk]?)/i);
  if (!m) return null;
  let v = parseFloat(m[2]);
  if (!Number.isFinite(v)) return null;
  if (m[3].toLowerCase() === "m") v *= 1_000_000;
  else if (m[3].toLowerCase() === "k") v *= 1_000;
  return m[1] === "-" ? -v : v;
}
const TEAM_SEASON_CSV = "team_season_stats.csv";
const TEAM_MATCH_CSV = "team_match_stats.csv";
function findAdvancedCsv(): string | null {
  if (!fs.existsSync(DATA)) return null;
  const files = fs.readdirSync(DATA).filter((f) => /premier_league_complete_stats/i.test(f));
  if (files.length === 0) return null;
  return files.find((f) => /35thGameDay/i.test(f)) ?? files.sort((a, b) => b.length - a.length)[0];
}
...
async function main() {
  ...
  for (const c of clubRows) {
    ...
    const club: MergedClub = {
      ...
      dna: null,
      netTransferEur: parseNetTransfer(c.net_transfer_record),
      squadValueEur: null,
      recentMaxFeeEur: null,
      recentAvgFeeEur: null,
      recentDealCount: null,
    };
    clubs.push(club);
  ...
  for (const p of playerRows) {
    ...
    players.push({
      ...
      performance: null,
      advanced: null,
      contractExpiration: p.contract_expiration_date || null,
    });
  }
  ...
  // PL PREMIUM LAYER — real Sofascore advanced stats on PL players.
  let advMatched = 0;
  const advCsv = findAdvancedCsv();
  if (advCsv) {
    const adv = buildAdvancedIndex(path.join(DATA, advCsv));
    for (const p of activePlayers) {
      if (p.leagueId !== "GB1") continue; // PL only
      const hit = adv.byName.get(normalizeName(p.name));
      if (hit) {
        p.advanced = hit;
        advMatched++;
      }
    }
    console.log(`• Sofascore PL advanced matched: ${advMatched}`);
  } else {
    console.log(`• No premier_league_complete_stats CSV in data/ — skipping advanced layer.`);
  }

  // PL PREMIUM LAYER — data-derived Club DNA on PL clubs.
  const seasonPath = path.join(DATA, TEAM_SEASON_CSV);
  const matchPath = path.join(DATA, TEAM_MATCH_CSV);
  if (fs.existsSync(seasonPath) && fs.existsSync(matchPath)) {
    const dnaMap = buildClubDna(seasonPath, matchPath);
    let dnaAttached = 0;
    for (const c of activeClubs) {
      const key = plClubKey(c.name);
      const dna = key ? dnaMap.get(key) : undefined;
      if (dna) {
        c.dna = dna;
        dnaAttached++;
      }
    }
    console.log(`• Club DNA attached to ${dnaAttached} PL clubs`);
  } else {
    console.log(`• team_season_stats.csv / team_match_stats.csv not found — skipping Club DNA.`);
  }

  // Squad value per club (wealth proxy for feasibility) = sum of active players' MV.
  const svByClub = new Map<string, number>();
  for (const p of activePlayers) {
    if (p.marketValue > 0) svByClub.set(p.clubId, (svByClub.get(p.clubId) ?? 0) + p.marketValue);
  }
  for (const c of activeClubs) {
    const sv = svByClub.get(c.clubId);
    if (sv) c.squadValueEur = sv;
  }

  // Real recent spend per club from transfers.csv (same club_id space) — the
  // best available "single-signing capacity" signal (validated vs squad-value).
  const transfersPath = path.join(DATA, "transfers.csv");
  if (fs.existsSync(transfersPath)) {
    const trows = Papa.parse<Record<string, string>>(fs.readFileSync(transfersPath, "utf8"), {
      header: true,
      skipEmptyLines: true,
    }).data;
    const nowYear = new Date().getFullYear();
    const feesByClub = new Map<string, number[]>();
    for (const t of trows) {
      const fee = Number(t.transfer_fee);
      if (!Number.isFinite(fee) || fee <= 0) continue;
      const yr = Number((t.transfer_date || "").slice(0, 4));
      if (!Number.isFinite(yr) || yr < nowYear - 5 || yr > nowYear + 1) continue; // recent, ignore future-dated quirks
      const cid = t.to_club_id;
      if (!cid) continue;
      const arr = feesByClub.get(cid);
      if (arr) arr.push(fee);
      else feesByClub.set(cid, [fee]);
    }
    let attached = 0;
    for (const c of activeClubs) {
      const fees = feesByClub.get(c.clubId);
      if (!fees || fees.length === 0) continue;
      fees.sort((a, b) => b - a);
      c.recentMaxFeeEur = Math.round(fees[0]);
      c.recentAvgFeeEur = Math.round(fees.slice(0, 3).reduce((s, x) => s + x, 0) / Math.min(3, fees.length));
      c.recentDealCount = fees.length;
      attached++;
    }
    console.log(`• Real recent spend attached to ${attached} clubs (from transfers.csv)`);
  } else {
    console.log(`• transfers.csv not found — feasibility will use squad-value budget proxy.`);
  }

  // League baselines, TIER-KEYED: `${tier}:${role}:${dim}`. Each tier (sofa /
  // fbref / fc) has its own scale, so comparisons never cross tiers.
  const buckets = new Map<string, number[]>();
  for (const p of activePlayers) {
    for (const tier of TIER_ORDER) {
      for (const k of STYLE_KEYS) {
        const v = styleValueAtTier(p, k, tier as DimTier);
        if (v !== null) {
          const key = `${tier}:${p.role}:${k}`;
  ...
  // Real-rating baseline per role (PL only) for the Overall Quality factor.
  const ratingBuckets = new Map<string, number[]>();
  for (const p of activePlayers) {
    if (p.advanced && p.advanced.rating != null) {
      const key = `sofa:${p.role}:rating`;
      const arr = ratingBuckets.get(key);
      if (arr) arr.push(p.advanced.rating);
      else ratingBuckets.set(key, [p.advanced.rating]);
    }
  }
  for (const [key, vals] of ratingBuckets) {
